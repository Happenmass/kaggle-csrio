...
def main():
    ...
    # NSW holdout: train on 非NSW，验证在 NSW
    train_df = wide[wide["state"] != "NSW"].reset_index(drop=True)
    val_df = wide[wide["state"] == "NSW"].reset_index(drop=True)
    print(f"Train rows (non-NSW): {len(train_df)}, Val rows (NSW): {len(val_df)}")
    ...
    args_tr = TrainingArguments(
        output_dir=str(cfg.output_dir / "nsw_holdout"),
    ...
    print(f"NSW holdout eval metrics: {metrics}")

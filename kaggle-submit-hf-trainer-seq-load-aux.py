...
# =============================================================================
KAGGLE_DATA_DIR = Path("./")  # 里面有 csiro-biomass/test.csv + test/...
KAGGLE_VIT_DIR = Path("timm/vit_7b_patch16_dinov3.lvd1689m")  # 可选：本地 vit 权重目录（里面有 model.safetensors）
KAGGLE_HEAD_5FOLD_DIR = Path("runs/hf_trainer_aux_test_exp")  # A 格式目录
EXP_MODE = "exp4"  # exp1 ~ exp7
NONNEG_CLAMP = False  # 推导后的目标值是否裁剪为非负
LOG1P_TARGETS = False  # 若训练时启用 log1p 目标，这里也需同步为 True
...
# %%
# =============================================================================
# Head 结构（为保证脚本独立运行，这里直接内置一份与 train_hf_trainer_aux_test.py 对齐的实现）
# =============================================================================
@dataclass
class HFConfig:
    dropout: float = 0.1
...
def _head_forward_to_logits(
    ...
    preds = out["preds"]
    if LOG1P_TARGETS:
        preds = {k: torch.expm1(v) for k, v in preds.items()}
